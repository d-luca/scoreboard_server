...
4. Leave *Shutdown source when not visible* **off** and *Refresh browser when
   scene becomes active* on if you like — the page reconnects by itself.
...
*Private networks*), that both devices are on the same network, and that the
...
If `pnpm dev` opens a window that only shows *"WebKit encountered an internal
error"* and the backend console logs a `symbol lookup error` in
